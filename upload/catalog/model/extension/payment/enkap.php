...
class ModelExtensionPaymentEnkap extends Model
{
    ...
    {
        return $this->db->query("SELECT `order_transaction_id`, `order_id` FROM `" . DB_PREFIX . "enkap_transaction` WHERE `merchantReference` = '" . $this->db->escape($merchantReference) . "'")->row;
    }
